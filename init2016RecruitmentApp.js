...
Program.findOne({ shortname: 'recruitment' }, function(error, recruitmentProgram) {
    ...
    recruitmentApp.save(function (err, newApplication) {
        if (err) {
            ...
            console.log('Successfully initialized 2016 fall recruitment application.');
            Program.update(
                {
                ...
                function(err) {
                    ...
                    else console.log('Recruitment currentlyLiveApplication set to ' + newApplication._id);
                    process.exit();
                }
            );
        }
